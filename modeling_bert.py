...
class BertForSequenceClassification(BertPreTrainedModel):
    ...
    Examples::
        ...
        pooled_output = outputs[1]
        last_h = outputs[0] # last_hidden_state
        # a = last_h[:, :, 0]
        pooled_output = self.dropout(pooled_output)
        h_avg = torch.max_pool1d(last_h, 2)
        h_max = torch.avg_pool1d(last_h, 2)
        h_max_avg = torch.cat([h_max, h_avg], 2)[:,0]
        re = pooled_output+h_max_avg
        logits = self.classifier(re)
