...
class RobertaForSequenceClassification(BertPreTrainedModel):
    ...
    Examples::
        ...
        last_cat = torch.cat((outputs[2][-1], outputs[2][-2], outputs[2][-3]), 2)
...
class RobertaClassificationHead(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        new_parameter = config.hidden_size * 3
        self.dense = nn.Linear(new_parameter, new_parameter)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.out_proj = nn.Linear(new_parameter, config.num_labels)
